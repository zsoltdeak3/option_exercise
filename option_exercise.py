...
if st.session_state['example'] == "Single instrument":
  ...
  if exercise_button:
    ...
    if moneyess_perc >= threshold:
      ...
      st.session_state['pre_ccp_pos2'] = pd.DataFrame({'Moneyness':[moneyess_perc],'Settlement':[st.session_state['ccp_settlement']]}).reset_index(drop=True)
      ...
      #st.session_state['ccp_pos2'] =
      colu1.dataframe(st.session_state['pre_ccp_pos2'], use_container_width=True)
